...
function data_plane_build_push() {
  ...
  export KNATIVE_KAFKA_SINK_RECEIVER_IMAGE="${KNATIVE_KAFKA_BROKER_RECEIVER_IMAGE}"
  export KNATIVE_KAFKA_CHANNEL_RECEIVER_IMAGE="${KNATIVE_KAFKA_BROKER_RECEIVER_IMAGE}"

  export KNATIVE_KAFKA_BROKER_DISPATCHER_IMAGE="${KO_DOCKER_REPO}"/"${dispatcher}":"${uuid}"
	export KNATIVE_KAFKA_CHANNEL_DISPATCHER_IMAGE="${KNATIVE_KAFKA_BROKER_DISPATCHER_IMAGE}"
